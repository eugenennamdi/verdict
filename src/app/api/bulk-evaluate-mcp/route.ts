...
const verifyTransactionManually = async (txHash: string): Promise<boolean> => {
  try {
    ...
    if (tx.to?.toLowerCase() !== USDT_CONTRACT) return false;

    const input = tx.input || "";
    if (!input.startsWith("0xa9059cbb") || input.length < 138) return false;

    const paddedRecipient = "0x" + input.substring(10, 74).slice(-40).toLowerCase();
    if (paddedRecipient !== PAYMENT_ADDRESS) return false;
